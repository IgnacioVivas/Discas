...
const DonateSection = () => {
	return (
		<div
			className="w-full flex flex-col justify-center gap-3 p-20"
			style={{ backgroundImage: `url(${bgPatitas.src})` }}
		>
			<div className="px-20 py-10 bg-teal-950 bg-opacity-85 rounded-xl">
				<h2 className={`text-4xl text-center text-white font-semibold mb-5 ${poppins.className}`}>
					Haz una diferencia hoy
				</h2>
				<h3 className="text-lg text-center text-white font-normal">
				...
				<div className="grid grid-cols-2 gap-9 justify-items-center md:grid-cols-3 text-white font-bold text-2xl">
					{['$6.000', '$8.000', '$12.000', '$18.000', '$24.000', '$30.000'].map((price, index) => (
						<motion.div
							key={index}
							className="w-full py-5 bg-teal-400 rounded-xl text-center border-2 border-stone-900"
							whileHover={{
								scale: 1.1,
								rotate: [0, 3, -3, 0],
				...
				<div className="grid grid-cols-2 gap-4 md:grid-cols-4 text-white font-bold text-2xl">
					{['$5000', '$10000', '$15000', '$25000'].map((price, index) => (
						<motion.div
							key={index}
							className="py-3 bg-teal-400 rounded-xl text-center border-2 border-stone-900"
							whileHover={{
								scale: 1.1,
								rotate: [0, 3, -3, 0],
								boxShadow: '0px 4px 30px rgba(0, 128, 128, 0.8)',
				...
				<div className="grid grid-cols-1 md:grid-cols-2 gap-9 mt-9">
					...
					>
						<h2 className="font-semibold text-lg">APORTÁ USANDO ALIAS</h2>
						<div className="font-bold text-3xl text-teal-400">CABRA.OSO</div>
					</div>
					<div
						className={`text-white flex flex-col justify-center items-center gap-2 bg-teal-950/70 rounded-xl py-5 ${nunito.className}`}
					>
						<h2 className="uppercase font-semibold text-lg">O SI ESTÁS FUERA DE ARGENTINA</h2>
						<div className="font-bold text-3xl text-teal-400">PayPal</div>
					</div>
				</div>
			</div>
		</div>
